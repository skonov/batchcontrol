...
public class BatchControlServiceImpl {
	...
	/**
	 * Reloads all batches and schedulers from database only if no process is
	 * running and no batch is active.
	 * 
	 * @throws Exception
	 *             if there is any process running or any batch is active.
	 */
	...
	public synchronized void startBatch(String taskClassName, boolean runOnce) throws Exception {
		synchronized (monitor) {
			...
			try {
				...
				String result = batch.start() ? "Batch started: " + batch.getName()
						: "Batch NOT started: " + taskClassName;
	...
	public synchronized void updateScheduler(int id, String type, int hour, int minute, int second, String days)
			throws Exception {
		Connection con = null;
		PreparedStatement stmt = null;
		try {
			...
			stmt.setInt(6, id);
			stmt.execute();
			log.debug("Scheduler updated: id=" + id + ", type=" + type + ", hour=" + hour + ", min=" + minute + ", sec="
					+ second + ", days=" + days);
		} catch (Exception e) {
	...
	/**
	 * Creates iterator for the specified batch in database. It does not update
	 * current service state. After the iterator has been created batches can be
	 * reloaded and the newly created iterator will be registered by the service.
	 * Return database ID of newly inserted scheduler.
	 * 
	 * @param type
	 *            iterator type: H(every hour), M(every minute), D(daily),
	 *            R(restricted daily), O(monthly), F(first business day of month)
	 * @param hour
	 *            hour (0-24)
	 * @param minute
	 *            minute
	 ...
	 *            second
	 * @param days
	 *            arrays of integers representing days of week, where 1 is Sunday, 2
	 *            - Monday, and so on.
	 * @param batchName
	 *            short batch name.
	 * 
	 ...
	 *             if database error occurs.
	 */
	public synchronized int createSchedulerIterator(String type, int hour, int minute, int second, String days,
			String batchName) throws Exception {
		Connection con = null;
		PreparedStatement stmt = null;
	 ...
	 * After the batch has been created batches can be reloaded and the newly
	 * created batch will be registered by the service.
	 * 
	 * @param name
	 *            short batch name.
	 * @param taskClassName
